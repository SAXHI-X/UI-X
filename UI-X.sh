#!/bin/bash
# @fileOverview Check usage stats of X-SACHI
# @author MasterHide
...
show_menu() {
    echo "Welcome to UI-X Installer/Uninstaller"
    echo "Please choose an option:"
    echo "1. Run UI-X (Install)"
    echo "2. Uninstall UI-X"
    echo "3. Exit"
}
...
while true; do
    ...
    case $CHOICE in
        1)
            echo "Proceeding with UI-X installation..."
            break
            ;;
        2)
            echo "Uninstalling UI-X..."
            bash <(curl -s https://raw.githubusercontent.com/SAXHI-X/UI-X/main/rm-UX.sh)
            echo "UI-X has been uninstalled."
            exit 0
            ;;
        3)
            echo "Exiting..."
...
echo "Downloading UI-X version $VERSION..."
if [ "$VERSION" == "latest" ]; then
    DOWNLOAD_URL="https://github.com/SAXHI-X/UI-X/archive/refs/heads/main.zip"
else
    DOWNLOAD_URL="https://github.com/SAXHI-X/UI-X/archive/refs/tags/$VERSION.zip"
fi

cd /home/$USERNAME
if curl -L "$DOWNLOAD_URL" -o UI-X.zip; then
    echo "Download successful. Extracting files..."
    unzip -o UI-X.zip -d /home/$USERNAME
    EXTRACTED_DIR=$(ls /home/$USERNAME | grep "UI-X-" | head -n 1)
    mv "/home/$USERNAME/$EXTRACTED_DIR" /home/$USERNAME/UI-X
    rm UI-X.zip
else
    echo "Failed to download UI-X version $VERSION. Exiting."
    exit 1
fi

# Verify the templates directory exists
if [ -d "/home/$USERNAME/UI-X/templates" ]; then
...
mkdir -p /var/lib/UI-X/certs
sudo chown -R $USERNAME:$USERNAME /var/lib/UI-X/certs

# Check if valid certificate already exists
if [ -f "/var/lib/UI-X/certs/$DOMAIN.cer" ] && [ -f "/var/lib/UI-X/certs/$DOMAIN.cer.key" ]; then
    echo "Valid SSL certificate already exists."
    SSL_CONTEXT="--certfile=/var/lib/UI-X/certs/$DOMAIN.cer --keyfile=/var/lib/UI-X/certs/$DOMAIN.cer.key"
else
    echo "Generating SSL certificate..."
    curl https://get.acme.sh | sh -s email=$USERNAME@$SERVER_IP
    ~/.acme.sh/acme.sh --issue --force --standalone -d "$DOMAIN" \
        --fullchain-file "/var/lib/UI-X/certs/$DOMAIN.cer" \
        --key-file "/var/lib/UI-X/certs/$DOMAIN.cer.key"
    # Fix ownership of the generated certificates
    sudo chown $USERNAME:$USERNAME /var/lib/UI-X/certs/$DOMAIN.cer
    sudo chown $USERNAME:$USERNAME /var/lib/UI-X/certs/$DOMAIN.cer.key
...
if sudo systemctl is-active --quiet UI-X; then
    echo "Stopping existing UI-X service..."
    sudo systemctl stop UI-X
fi

# Create a systemd service to keep the Flask app running with Gunicorn
echo "Setting up systemd service..."
cat > /etc/systemd/system/UI-X.service <<EOL
[Unit]
Description=UI-X Web App
After=network.target

[Service]
User=$USERNAME
WorkingDirectory=/home/$USERNAME/UI-X
ExecStart=/bin/bash -c 'source /home/$USERNAME/UI-X/venv/bin/activate && exec gunicorn -w 4 -b 0.0.0.0:$PORT $SSL_CONTEXT app:app'
Environment="DB_PATH=/etc/x-ui/x-ui.db"
Restart=always
RestartSec=5
StandardOutput=append:/var/log/UI-X.log
StandardError=append:/var/log/UI-X.log
SyslogIdentifier=UI-X
...
sudo systemctl enable UI-X
sudo systemctl start UI-X
